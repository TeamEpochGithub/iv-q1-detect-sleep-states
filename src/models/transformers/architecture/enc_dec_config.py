...
class EncDecConfig(nn.Module):
    ...
    :param n_layers: Number of encoder and decoder layers.
    ...
    def __init__(self, tokenizer: str = "patch", tokenizer_args: dict = {}, pe: str = "fixed",
        ...
        self.encoder = Encoder(tokenizer=self.tokenizer, pe=self.pe, emb_dim=emb_dim, forward_dim=forward_dim, n_layers=n_layers, heads=heads)
        self.decoder = Decoder(tokenizer=self.tokenizer, pe=self.pe, emb_dim=emb_dim, forward_dim=forward_dim, n_layers=n_layers, heads=heads)
        self.out = nn.Linear(emb_dim, seq_len)
